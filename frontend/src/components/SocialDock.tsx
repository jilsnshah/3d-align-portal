...
/* A message that names the caller. The lab reads these on a phone with no
   context, so the doctor's own name and clinic save a round trip asking who
   this is. */
function whatsappHref(name?: string, clinic?: string): string {
  const who = [name, clinic].filter(Boolean).join(", ");
  const text = who
    ? `Hi 3D Align, this is ${who}. I have a question about a case.`
    : "Hi 3D Align, I have a question about a case.";
