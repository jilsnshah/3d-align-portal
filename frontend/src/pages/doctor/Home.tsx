...
/* --- the practice, drawn as an arch -----------------------------------------
   The aligner journey laid along the curve of an upper arch, lit gold where
   the clinic is the one holding cases up. The lab's queue draws the same arch
   from its own side of the counter. */
function PracticeArch({ orders, loading }: { orders: OrderSummary[]; loading: boolean }) {
  const navigate = useNavigate();
  const { stages, other } = useMemo(() => stageCounts(orders, (o) => o.needs_doctor_action), [orders]);
  return (
    <ArchPipeline
      stages={stages}
      loading={loading}
      label="Where your aligner cases stand"
      centerLabel="Aligner cases in treatment"
      centerNote={other > 0 ? `and ${other} product ${other === 1 ? "order" : "orders"}` : undefined}
      litNote={(n) => `${n} with you`}
      keys={["Waiting on you", "With 3D Align"]}
      onPick={() => navigate("/orders")}
    />
  );
}
