 ...
 *
 * It opens on the day's one fact, addressed to the doctor — how many cases are
 * waiting on them, and which to start with — beside a picture of the practice
 * drawn as an arch: every stage of the aligner journey a point on the curve,
 * sized by how many cases stand there and lit where they wait on the clinic.
 * The range follows, shown as the lab's own product shots rather than its
 * flyers, and the desk underneath holds what needs them and what has moved.
 *
...
/** The single most pressing thing, as a bar under the headline so it can be
    acted on without scrolling. The full list is on the desk. */
  ...
  return (
    <div className="hm-next">
      ...
      <button type="button" className="hm-next-go" onClick={() => navigate(to)}>
        {top.cases.length === 1 ? "Open" : "Work through"}
        <Arrow />
      </button>
    </div>
  );
...
/* --- the practice, drawn as an arch -----------------------------------------
   The aligner journey laid along the curve of an upper arch: each stage a
   point on it, sized by how many of the clinic's cases stand there, and lit
   gold where the clinic is the one holding them up. It says in one look what
   the figures below say in four, and it is the one picture on the page that is
   about this practice rather than about the lab. */
...
type StageCount = { key: string; label: string; total: number; you: number };

/** Open aligner cases counted by stage. A case that came in with its own scan
    has no quote stage, and its stages are matched to the full journey by key,
    so it lands on the same point as every other case at that step. */
function pipeline(orders: OrderSummary[]): { stages: StageCount[]; other: number } {
  const counts = new Map<string, StageCount>(
    stagesFor("ALIGNER").map((s) => [s.key, { key: s.key, label: s.label, total: 0, you: 0 }]),
  );
  let other = 0;
  for (const o of orders) {
    ...
    if (!c) continue;
    c.total += 1;
    if (o.needs_doctor_action) c.you += 1;
  }
  return { stages: [...counts.values()], other };
}

function PracticeArch({ orders, loading }: { orders: OrderSummary[]; loading: boolean }) {
  const navigate = useNavigate();
  const { stages, other } = useMemo(() => pipeline(orders), [orders]);
  ...
  return (
    <figure className="hm-pipe" role="group" aria-label="Where your aligner cases stand">
      ...
      <div className="pipe-center" style={{ "--top": `${((cy - 118) / H) * 100}%` } as CSSProperties}>
        <b>{loading ? "—" : total}</b>
        <span>Aligner cases in treatment</span>
        {other > 0 && (
          <small>
            and {other} product {other === 1 ? "order" : "orders"}
          </small>
        )}
      </div>
      ...
      {stages.map((s, i) => {
        ...
        const state = s.you > 0 ? "you" : s.total > 0 ? "lab" : "none";
        const title =
          `${s.total} at ${s.label}` + (s.you > 0 ? ` — ${s.you} waiting on you` : "");
        return (
          <button
            key={s.key}
            type="button"
          ...
          >
            ...
            <span className="pipe-say">
              <b>{s.label}</b>
              {s.you > 0 && <small>{s.you} with you</small>}
            </span>
          </button>
        );
      })}
      ...
      <figcaption className="hm-pipe-key">
        <span className="k-you">Waiting on you</span>
        <span className="k-lab">With 3D Align</span>
      </figcaption>
    </figure>
  );
}
    ...
    <main className="page hm">
      <section className="hm-hero">
        <div className="hm-say">
          ...
          {/* The headline is the day's one fact, addressed to the doctor. The
              name on its own told them nothing they did not already know. */}
          <h1>
            {firstName(me?.doctor?.full_name ?? "Doctor")},{" "}
            {!loaded ? (
              "welcome back."
            ) : needing > 0 ? (
              <>
                <em>
                  {needing} {needing === 1 ? "case is" : "cases are"}
                </em>{" "}
                waiting on you.
              </>
            ) : (
              <>
                everything is <em>with the lab.</em>
              </>
            )}
          </h1>
        ...
        <PracticeArch orders={openCases.data ?? []} loading={openCases.isLoading} />
      </section>
